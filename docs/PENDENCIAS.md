...
- [ ] **Cálculo de multa e juros:** o exemplo da planilha traz multa e juros já calculados na origem. Confirmar se o EthosFinancial deve **recalcular** esses valores (com fórmula própria) ou apenas **exibir** o que vier importado da planilha. **Atualização 2026-07-03:** o "Total devedor" do relatório de inadimplência (Sprint 3) hoje é só a soma de `Parcela.valor` das parcelas vencidas — sem multa/juros, porque o schema não tem campos para isso e a fórmula não está definida. Isso deixa o total abaixo do valor real de cobrança/protesto até essa decisão ser tomada.
...
- [ ] **Modelo jurídico real ainda não recebido (2026-07-03):** implementamos um layout **provisório** em `apps/api/src/modules/relatorios/documento-protesto.generator.ts` a partir só da descrição textual do PRD (seção 16), sem nenhum arquivo de modelo jurídico para replicar. CLAUDE.md (seção 8) diz para não alterar o layout sem confirmação — isso vale a partir de quando o modelo real chegar; até lá, este layout deve ser tratado como rascunho, não como o padrão final. Também falta identidade da instituição (nome/CNPJ do credor) em `Configuracao` — hoje vem de env (`INSTITUICAO_NOME`/`INSTITUICAO_CNPJ`), fora do padrão de configuração do sistema.
