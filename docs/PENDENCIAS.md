...
- [ ] **Cálculo de multa e juros:** **atualização 2026-07-03** — comparando os 4 documentos de protesto reais fornecidos pelo usuário (`C:\DSI\Git\CapturaMemoriaCalculo\relatorio\*.docx`), reverso-engenheirei a fórmula: **Multa = 2% flat sobre o valor bruto da parcela** (bate exato em todos os exemplos) e **Juros = 1% ao mês, pro-rata por dia de atraso** (base 30 dias — fica próximo dos exemplos, mas não bate ao centavo; ex.: parcela de R$213,64 com 118 dias de atraso deu juros de R$8,32 no exemplo real vs. R$8,40 pela minha fórmula — diferença de poucos centavos, possivelmente contagem de dias ou arredondamento intermediário diferente no sistema de origem). Implementado em `apps/api/src/modules/relatorios/calculo-financeiro.ts` e já é o valor usado no relatório de inadimplência e no documento de protesto (Sprint 3). **Precisa confirmação da instituição**: (a) a fórmula 2% multa + 1% a.m. juros está correta, e (b) qual a regra exata de contagem de dias/arredondamento para bater ao centavo.
