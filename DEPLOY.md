...
| `ng build` termina com `Killed` / código 137 no `scripts/deploy.sh`       | OOM — o kernel matou o processo por falta de RAM (build de produção do Angular pode picar 1.5-2GB; VPS com pouca RAM e sem swap não aguenta)        | `free -h` para confirmar; se não tiver swap, criar um (`sudo fallocate -l 4G /swapfile && sudo chmod 600 /swapfile && sudo mkswap /swapfile && sudo swapon /swapfile` — adicionar em `/etc/fstab` para persistir no boot) e rodar o deploy de novo |
